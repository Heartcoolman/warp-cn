...
impl App {
    ...
    pub fn rebuild_main_menu(&self, ctx: &mut AppContext) {
        if let Some(ref builder) = self.menu_bar_builder {
            ...
            // SAFETY: locale-change menu rebuilds are dispatched on the main
            // thread; `make_main_menu` builds AppKit objects that must run there.
            let mtm = unsafe { MainThreadMarker::new_unchecked() };
            let nsmenu = unsafe { make_main_menu(menu_bar) };
            NSApplication::sharedApplication(mtm).setMainMenu(Some(&nsmenu));
        }
    }
}
