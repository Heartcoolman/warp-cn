...
ai-ui-auto-approve-locked = 云端智能体对话始终启用快进
...
ai-ui-at-context-ssh = 不支持没有远程服务器的 SSH 会话
...
ai-ui-agent-tip-handoff = 输入 `&` 或使用交接（handoff）标记，即可将本地会话转移到云端。
...
ai-ui-agent-source-orchestration = 编排任务
...
ai-ui-conversation-details-initial-query = 初始提问
...
ai-ui-agent-source-run-scorer = 评分器
ai-ui-agent-source-autofix = 自我改进
ai-ui-agent-source-benchmark = 基准测试
