...
impl UserWorkspaces {
    ...
    /// path is the entire point — unconditionally unlock the upstream BYOK paywall (for
    /// logged-in users) so they can fill in OpenAI / Anthropic / Google keys via the settings
    /// UI without subscribing to Warp's Build plan. Upstream behavior is preserved when the
    /// feature is off.
