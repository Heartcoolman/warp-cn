...
pub(crate) fn initialize_app(
    ...
    ctx.add_singleton_model(|ctx| {
        // Fork: backend-aware gating. `is_codebase_context_enabled_for_indexing`
        // already folds in upstream's `UserWorkspaces::is_codebase_context_enabled`
        // for the WarpCloud backend; OR in upstream's RemoteServerDaemon clause so
        // the daemon still restores indices.
        let should_restore_indices = launch_mode.supports_indexing()
            && (matches!(launch_mode, LaunchMode::RemoteServerDaemon { .. })
                || is_codebase_context_enabled_for_indexing(ctx));
        ...
        // Fork: backend-aware limits (Auggie local defaults vs. upstream's
        // server-driven `AIRequestUsageModel::codebase_context_limits()`).
