    ...
    fn handle_result_selected(
    ...
    ) {
        {
            ...
            let (a11y_content, a11y_help_content) = if was_immediately_executed {
                (
                    warp_i18n::t!("a11y-command-search-result-executed-label").to_string(),
                    warp_i18n::t!("a11y-command-search-result-executed-help").to_string(),
                )
            } else {
                (
                    warp_i18n::t!("a11y-command-search-result-accepted-label").to_string(),
                    warp_i18n::t!("a11y-command-search-result-accepted-help").to_string(),
                )
            };
...
impl View for CommandSearchView {
    ...
    fn accessibility_contents(&self, _ctx: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            warp_i18n::t!("a11y-command-search-label").to_string(),
            warp_i18n::t!("a11y-command-search-help").to_string(),
