...
fn user_facing_git_error(raw: &str) -> String {
    ...
    if lower.contains("no changes added to commit") {
        ...
        warp_i18n::t!("code-review-git-error-no-staged-changes")
    } else if lower.contains("nothing to commit") {
        warp_i18n::t!("code-review-git-error-no-changes")
    } else if lower.contains("please tell me who you are")
        || lower.contains("author identity unknown")
    {
        warp_i18n::t!("code-review-git-error-identity-not-configured")
    } else if lower.contains("updates were rejected")
        || lower.contains("non-fast-forward")
        || lower.contains("fetch first")
    {
        warp_i18n::t!("code-review-git-error-pull-before-push")
    ...
    {
        warp_i18n::t!("code-review-git-error-no-remote")
    } else if lower.contains("authentication failed")
        || lower.contains("permission denied (publickey)")
    {
        warp_i18n::t!("code-review-git-error-auth-failed")
    } else if lower.contains("could not resolve host")
        || lower.contains("network is unreachable")
        || lower.contains("connection timed out")
    {
        warp_i18n::t!("code-review-git-error-network")
    } else if lower.contains("repository not found") {
        warp_i18n::t!("code-review-git-error-repo-not-found")
        ...
        warp_i18n::t!("code-review-git-error-gh-cli-missing")
        ...
        || lower.contains("gh auth login")
    {
        ...
        warp_i18n::t!("code-review-git-error-gh-not-authenticated")
        ...
        warp_i18n::t!("code-review-git-error-git-operation-in-progress")
    } else {
        warp_i18n::t!("code-review-git-error-generic")
    }
}
...
fn render_file_changes_box(
    ...
    let files_text = Text::new(
        warp_i18n::t!("code-review-git-files-count", count = total_files),
        appearance.ui_font_family(),
        appearance.ui_font_size(),
    )
    .with_color(main_color)
...
impl GitDialog {
    pub fn new_for_commit(
        ...
        let (confirm_button, cancel_button, close_button) =
            Self::build_dialog_buttons(warp_i18n::t!("code-review-git-button-confirm"), None, ctx);
    ...
    fn build_dialog_buttons(
        ...
        let cancel_button = ctx.add_typed_action_view(|_ctx| {
            ActionButton::new(warp_i18n::t!("code-review-git-button-cancel"), NakedTheme)
                .with_size(ButtonSize::Small)
                .with_height(32.)
    ...
    fn title(&self) -> String {
        match &self.mode {
            GitDialogMode::Commit(_) => warp_i18n::t!("code-review-git-title-commit"),
            GitDialogMode::Push(state) => {
                if state.publish {
                    warp_i18n::t!("code-review-git-title-publish-branch")
                } else {
                    warp_i18n::t!("code-review-git-title-push-changes")
                }
            }
            GitDialogMode::CreatePr(_) => warp_i18n::t!("code-review-git-title-create-pr"),
        }
    }
