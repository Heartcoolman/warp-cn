...
impl CreateApiKeyModal {
    ...
    fn render_success_content(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let info = Text::new(
            warp_i18n::t!("settings-platform-api-key-secret-info"),
        ...
        match self.request_state {
            ...
            _ => {
                ...
                let name_label = Text::new(
                    warp_i18n::t!("settings-platform-api-key-name-label"),
                    appearance.ui_font_family(),
                    LABEL_FONT_SIZE,
                )
                .with_color(theme.active_ui_text_color().into())
                ...
                // Show segmented control only if user has a team or named agents
                if self.has_team || self.has_named_agents {
                    let type_label = Text::new(
                        warp_i18n::t!("settings-platform-api-key-type-label"),
                        appearance.ui_font_family(),
                        LABEL_FONT_SIZE,
                    )
                    .with_color(theme.active_ui_text_color().into())
                ...
                if selected_key_type == ApiKeyType::Agent {
                    let agent_label = Text::new(
                        warp_i18n::t!("settings-platform-api-key-agent-label"),
                        appearance.ui_font_family(),
                        LABEL_FONT_SIZE,
                    )
                    .with_color(theme.active_ui_text_color().into())
                    ...
                    if !self.is_loading_agents && available_agents.is_empty() {
                        let empty_text = Text::new(
                            warp_i18n::t!("settings-platform-no-agents-available"),
                            appearance.ui_font_family(),
                            LABEL_FONT_SIZE,
                        )
                        .with_color(theme.nonactive_ui_text_color().into())
                ...
                let expiration_label = Text::new(
                    warp_i18n::t!("settings-platform-api-key-expiration-label"),
                    appearance.ui_font_family(),
                    LABEL_FONT_SIZE,
                )
                .with_color(theme.active_ui_text_color().into())
