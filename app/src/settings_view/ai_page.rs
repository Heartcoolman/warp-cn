...
pub fn init_actions_from_parent_view<T: Action + Clone>(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-git-operations-autogen"),
                builder(SettingsAction::AI(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-include-agent-commands-in-history"),
                builder(SettingsAction::AI(
            ...
            .with_group(bindings::BindingGroup::WarpAi),
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-conversation-history-in-tools-panel"),
                builder(SettingsAction::AI(
            ...
            .with_group(bindings::BindingGroup::WarpAi),
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-model-picker-in-prompt"),
                builder(SettingsAction::AI(
            ...
            .with_group(bindings::BindingGroup::WarpAi),
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-coding-agent-toolbar"),
                builder(SettingsAction::AI(
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-rules"),
            ...
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-suggested-rules"),
                builder(SettingsAction::AI(
            ...
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-warp-drive-as-agent-context"),
                builder(SettingsAction::AI(
            ...
            .with_enabled(|| FeatureFlag::AIRules.is_enabled()),
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-auto-spawn-third-party-mcp"),
    ...
    ToggleSettingActionPair::add_toggle_setting_action_pairs_as_bindings(
        vec![
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-warp-credit-fallback"),
                builder(SettingsAction::AI(
            ...
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-auto-toggle-rich-input"),
                builder(SettingsAction::AI(
            ...
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-auto-open-rich-input-on-cli-agent-start"),
                builder(SettingsAction::AI(
            ...
            .with_enabled(|| FeatureFlag::CLIAgentRichInput.is_enabled()),
            ToggleSettingActionPair::new(
                &warp_i18n::t!("settings-ai-cmd-suffix-auto-dismiss-rich-input-after-submit"),
                builder(SettingsAction::AI(
...
impl AISettingsPageView {
    ...
    fn build_page(subpage: Option<AISubpage>, ctx: &mut ViewContext<Self>) -> PageType<Self> {
        ...
        let title = match subpage {
            Some(AISubpage::Knowledge) => Some(warp_i18n::t_static!("settings-ai-knowledge")),
            Some(AISubpage::ThirdPartyCLIAgents) => {
                Some(warp_i18n::t_static!("settings-ai-third-party-cli-agents"))
            }
...
impl ApiKeysWidget {
    fn new(ctx: &mut ViewContext<<Self as SettingsWidget>::View>) -> Self {
        ...
        // warp-cn fork: when the `direct_llm_backend` cargo feature is on, each
        // editor also mirrors the saved key into
        // `DirectBackendConfig.overrides_for(kind).api_key` so the multi-agent
        // stream driver (which reads from a process-global snapshot, not from
        // `ApiKeyManager`) sees the value without restart.
            ...
            .map(|provider| {
                ...
                ctx.subscribe_to_view(&editor, move |_, editor, event, ctx| {
                    if matches!(event, EditorEvent::Blurred | EditorEvent::Enter) {
                        ...
                        #[cfg(feature = "direct_llm_backend")]
                        if let Some(kind) = direct_provider_kind(provider) {
    ...
    fn render_provider_key_editors(
        ...
        for provider_editor in &self.provider_api_key_editors {
            // warp-cn fork: the three providers the fork localizes keep their
            // translated labels; anything upstream adds later falls back to the
            // generic "<provider> API key" label.
            let label = match provider_editor.provider {
                LLMProvider::OpenAI => warp_i18n::t!("settings-ai-openai-key"),
                LLMProvider::Anthropic => warp_i18n::t!("settings-ai-anthropic-key"),
                LLMProvider::Google => warp_i18n::t!("settings-ai-google-key"),
                provider => format!("{} API key", provider.display_name()),
            };
            ...
            // warp-cn fork: Base URL override rendered directly under the key
            // field of the provider it applies to.
            #[cfg(feature = "direct_llm_backend")]
            {
                let base_url_row = match provider_editor.provider {
                    ...
                    LLMProvider::Google => Some((
                        "Gemini Base URL (optional)",
                        self.google_base_url_editor.clone(),
                    )),
                    LLMProvider::Xai | LLMProvider::Unknown => None,
                };
                if let Some((base_url_label, base_url_editor)) = base_url_row {
                    column.add_child(self.render_api_key_input(
                        appearance,
                        base_url_label.to_string(),
                        provider_editor.provider,
...
/// warp-cn fork: maps an upstream [`LLMProvider`] onto the direct-backend
/// provider whose overrides mirror that provider's API key. Returns `None` for
/// providers the direct backend does not model.
#[cfg(feature = "direct_llm_backend")]
fn direct_provider_kind(provider: LLMProvider) -> Option<::ai::direct_backend::DirectProviderKind> {
    use ::ai::direct_backend::DirectProviderKind;
    match provider {
        LLMProvider::OpenAI => Some(DirectProviderKind::OpenAi),
        LLMProvider::Anthropic => Some(DirectProviderKind::Anthropic),
        LLMProvider::Google => Some(DirectProviderKind::Gemini),
        LLMProvider::Xai | LLMProvider::Unknown => None,
    }
}
