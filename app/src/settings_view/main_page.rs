...
impl MainSettingsPageView {
    pub fn new(ctx: &mut ViewContext<MainSettingsPageView>) -> Self {
        ...
        // Fork uses the minisign-verified GitHub-release updater instead of
        // upstream's `VersionInfoWidget`; `GithubVersionInfoWidget` resolves
        // its own version string, so it is always registered.
