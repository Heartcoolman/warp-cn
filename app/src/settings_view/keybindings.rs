...
pub fn search_placeholder() -> String {
    warp_i18n::t!("settings-keybindings-search-placeholder")
}
fn shortcut_conflict_warning_text() -> String {
    warp_i18n::t!("settings-keybindings-shortcut-conflict-warning")
}
const KEYBINDINGS_PAGE_SHORTCUT: &str = "workspace:toggle_keybindings_page";
fn cancel_button_text() -> String {
    warp_i18n::t!("settings-keybindings-button-cancel")
}
fn clear_button_text() -> String {
    warp_i18n::t!("settings-keybindings-button-clear")
}
fn save_button_text() -> String {
    warp_i18n::t!("settings-keybindings-button-save")
}
...
impl KeybindingRow {
    ...
    fn render_clicked(
        ...
        let conflict_warning_text = shortcut_conflict_warning_text();
        ...
        let press_new_shortcut_label = warp_i18n::t!("settings-keybindings-press-new-shortcut");
        let press_new_shortcut_text = render_text(&press_new_shortcut_label, None, appearance);
...
impl KeybindingsWidget {
    fn render_description(
        ...
        let add_custom_description = warp_i18n::t!("settings-keybindings-add-custom-description");
    ...
    fn render(
        ...
        let local_only_icon_state = if *CloudPreferencesSettings::as_ref(app).settings_sync_enabled
        {
            Some(LocalOnlyIconState::Visible {
                ...
                custom_tooltip: Some(warp_i18n::t!("settings-keybindings-not-synced-tooltip")),
            })
        } else {
            None
        };
        ...
        let subheader = render_sub_header(
            appearance,
            warp_i18n::t!("settings-keybindings-configure-header"),
            local_only_icon_state,
        );
        let description = self.render_description(view.bindings.as_ref(), appearance);

        let column_command_label = warp_i18n::t!("settings-keybindings-column-command");
        Flex::column()
            .with_child(subheader)
            .with_child(description)
            .with_child(render_columns(
                Container::new(render_text(
                    &column_command_label,
                    Some(UiComponentStyles {
